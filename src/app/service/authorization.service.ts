...
export class AuthorizationService {
  ...
  private getAuthorization(): Authorization {
    ...
    result.isAuthorized = tmp ? Boolean(tmp) : false;
    tmp = localStorage.getItem(this.BEARER_TOKEN)
    result.bearerToken = tmp ? tmp : '';
  ...
  private setAuthorization(isAuthorized: boolean, bearerToken: string, expireDays: number) {
    localStorage.setItem(this.IS_AUTHORIZED, String(isAuthorized));
    localStorage.setItem(this.BEARER_TOKEN, bearerToken);
    localStorage.setItem(this.EXPIRE_DATE, String(expireDays));
  }
